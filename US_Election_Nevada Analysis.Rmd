---
title: "US Election: Nevada results and Trump's Claims about Hispanic Support"
author: "OmaymaS"
date: "March 15, 2016"
...
This work focuses on the results of the US elections' primaries in Nevada. The Republican candidate Donald Trump highlighted that his victory with 46% in Nevada was supported by Hispanics/Lationos. There were different political analysis about what what nevada showed about Trump's Hispanic support and what it did not. The following analysis addresses these specific questions:
...
#adding a column with the winner in each state based on the fractional votes
res<-res %>%
        group_by(state,county,party) %>%
        mutate(winner= candidate[which.max(fraction_votes)])
...
facts <-county_facts %>%
        ...
        select(state_abbreviation=state_abbreviation,county=area_name, populaion2014=PST045214,
               ...
               highschool=EDU635213,highdegree=EDU685213,forborn=POP645213,
               ...
               under5=AGE135214,under18=AGE295214,over65=AGE775214)
...
#select top Democrat candidates
primdata_dem<-filter(primdata,candidate %in% c("Hillary Clinton","Bernie Sanders"))
...
As we can see, one main county in Nevada, "Clark", stands out in the data and it is obviously the one which was the reason for Trump's victory in Nevada. So we need to dig deeper and see if there is something special about it. So we'll extract some columns and calculate the following values:
...
* Clark is on the top of the grid, i.e. it has high percentage of hespanics, as mentioned earlier, and also the highest percentage of hispanic-owned firms in all the given counties(9.4%).
...
ggplot(data= filter(primdata_rep_Nevada, candidate=="Donald Trump") , aes(x=hispanic,y=firms.hispanic))+
        ...
        # facet_grid(candidate~state)+
        labs(title="Donald Trump's Nevada's Votes based on Hispanic-Owned Firms & Hispanics Percentage ")+
        scale_x_continuous(name="Hispanics Percentage")+
...
* If we have access to further data about the percentage of Hispanics who support Republicans in Nevada in General and Clark in specific, the origin of the voters, the nature of the Hispanic-owned firms, we might be able to gain more insights into the results in Nevada.